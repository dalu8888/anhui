...
public class SHA1Util {
	...
	public static boolean checkPWD(String pwd,String srcPWD){
		if(pwd==null||"".equals(pwd)||pwd.length()!=(2*BYTE_LEN)){
			logger.error("待验证口令为空或不符合长度要求!");
			return false;
		}
		if(srcPWD==null||"".equals(srcPWD)||srcPWD.length()!=(4*BYTE_LEN)){
			logger.error("原始口令为空或不符合长度要求，不能执行验证操作!");
			return false;
		}
...
//		System.out.println(SHA1Util.getFormattedText(SHA1Util.encode("111111".getBytes())));
